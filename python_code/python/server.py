    ...
    try:
        while True:
            ...
            if data[:7] == "request":
                ...
                if record.decode()!= 'None':
                    ...
                    if save:
                        ...
                        c.send('->[server] !!! Server Error: DUPLICATE CSR_ID/CSR or INVALID SIGNATURE\n'.encode('utf-8'))
                else:
                    c.send('->[server] !!! Server Error: None received\n'.encode('utf-8'))

                phase1_2 = time.time() - phase1_02
                phase1_tot = time.time() - phase1_0
                with open('results/time/time_server_phase1.txt', 'a') as file:
            ...
            # Client messages
                ...
                with open('results/time/time_server_phase2.txt', 'a') as file:
                    file.write(str(data) + '\t'
                                + str(phase2) + '\t'
                                + str(time_checkDB) + '\t'
                                + str(time_sign)
